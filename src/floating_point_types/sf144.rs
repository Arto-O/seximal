...
impl Sf144 {
    ...
    ///
    /// let x = Sf144::new(2.0);
    /// let abs_difference = (x.powf(Sf144::new(2.0)) - x * x).abs();
    ///
    /// assert!(abs_difference.value() <= f64::EPSILON);
    /// ```
    ...
    ///
    /// assert!(abs_difference_x.value() <= f64::EPSILON);
    /// assert!(abs_difference_y.value() <= f64::EPSILON);
    /// ```
